# 第09章 U-Boot调试命令：命令行是嵌入式开发的神器

## 为什么命令行这么重要

你可能会问，调试不是应该用 JTAG 吗？为什么要学命令行？说实话，我一开始也是这么想的。刚接触嵌入式开发的时候，我觉得有了调试器就够了，U-Boot 命令行不过是些杂乱无章的工具，敲来敲去多麻烦啊。

直到有一次，我在现场调试一块板子，JTAG 调试器忘了带，板子启动不起来卡在 U-Boot 阶段。那时候我才意识到，命令行不是备选方案，而是第一生产力。你想想，JTAG 要接十几根线，还要考虑供电、驱动、操作系统兼容性，而命令行只需要一根串口线就能完成 90% 的调试工作。更重要的是，命令行调试快，敲几个命令就能知道问题在哪里，比单步调试效率高多了。

还有一个原因是成本。JTAG 调试器动辄几百上千块，而串口线几块钱就能买到。对于初创团队或者个人开发者来说，这笔账很划算。而且，U-Boot 命令行还能帮你理解启动流程的每个细节，这些是调试器给不了的。

所以这篇文章的目标很明确：带你系统性地掌握 U-Boot 调试命令，从板子信息查询到网络启动，从环境变量管理到设备树操作，从常见错误排查到实战调试案例。学完之后，你会发现自己对 U-Boot 的理解上了一个台阶，很多以前看起来神秘的问题，现在用几个命令就能定位到。
...
串口工具：minicom / picocom
```

环境不完全一样也没关系，U-Boot 命令的基本逻辑是通用的，不同版本的差异主要在于某些高级命令的支持上。
