...
值得一提的是，ARM 社区并没有照搬 PowerPC 的设备树实现，而是在此基础上做了很多改进。他们定义了一套更加完善的**设备树绑定文档**，规范了各种设备的属性命名和格式。这些文档后来成为了多架构共享的规范，不仅仅是 ARM 在用。
...
ARM Linux 在早期没有采用设备树，导致了后来巨大的重构成本。这告诉我们：**技术债务是会累积利息的**。当你为了快速实现功能而选择"捷径"时，你实际上是在向未来借贷，而且利息非常高。

如果 ARM 社区在 2005 年左右就认真考虑设备树，而不是等到被 Linus 骂了才动手，后续的痛苦会少很多。但 hindsight 是 20/20 的，当时的开发者面临着出货压力、兼容性等问题，做出那种选择也是可以理解的。

### 什么时候该重构

设备树的引入也给我们一个启示：**重构要选对时机**。

ARM 社区在引入设备树时采取了渐进式策略，而不是"大爆炸"式的全面重写。这保证了：
1. 现有代码继续工作，不影响当前用户
2. 新代码使用新机制，逐步积累经验
3. 老代码有时间迁移，不至于烂尾

这种策略值得借鉴。当你决定重构一个系统时，不要想着一夜之间完成所有事情。给新旧机制一个共存期，让生态有时间适应。

### 标准化的价值

设备树最终能够跨架构普及，关键在于**标准化**。

ARM 社区没有搞自己的一套，而是采用了 PowerPC 的成熟方案，然后基于它制定了详细的绑定文档。这些文档后来成为了多架构共享的规范。

如果你在设计一个新系统，想想：它能被其他人复用吗？它值得被标准化吗？如果答案是肯定的，那就按照标准的方式来做，不要重新发明轮子。

### 技术选型的政治因素

最后，我们不得不承认：**技术演进不完全是技术问题**。

Linus 的那次"爆发"，对设备树在 ARM 上的普及起到了决定性作用。如果没有来自核心维护者的压力，ARM 社区可能会继续拖延下去，问题会越积越多。

这说明，推动技术变革有时候需要"独裁者"的存在。在一个民主松散的社区里，很难达成"为了长远利益而承受短期痛苦"的共识。这时候需要有人站出来，说一句："就这样了，必须改。"
...
- [05. 驱动与设备树的绑定](./05_driver_device_binding.md)：讲解如何在驱动中读取设备树属性，实现驱动的自动匹配。
