/**
 * @file pinctrl_gpio_demo_04_driver.c
 * @author Charliechen114514 (chengh1922@mails.jlu.edu.cn)
 * @brief LED driver using GPIO/Pinctrl Subsystem
 * @version 0.1
 * @date 2026-05-03
 *
...
// 模块退出
static void __exit pinctrl_gpio_demo_04_exit(void) {
    pr_info("=== pinctrl_gpio_demo_04驱动卸载成功 ===\n");
    release_led_handle(&led_handle);
    led_hw_deinit();
    pr_info("========================\n");
}

module_init(pinctrl_gpio_demo_04_init);
module_exit(pinctrl_gpio_demo_04_exit);

MODULE_LICENSE("GPL");
MODULE_AUTHOR("Charliechen114514");
MODULE_DESCRIPTION("Pin Control And GPIO Demo");
MODULE_VERSION("1.0");
