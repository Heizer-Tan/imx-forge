/**
 * @file mock_nonblocking_io_demo.c
 * @brief Mock nonblocking I/O demo device implementation for testing
 * @date 2026-05-10
 */

#include "mock_nonblocking_io_demo.h"
#include <errno.h>
#include <fcntl.h>
#include <stdio.h>
#include <stdlib.h>
...
mock_nonblocking_io_dev_t* mock_nonblocking_io_create(void) {
    mock_nonblocking_io_dev_t* dev = calloc(1, sizeof(mock_nonblocking_io_dev_t));
    if (!dev) {
        return NULL;
    }

    snprintf(dev->temp_path, sizeof(dev->temp_path), "/tmp/nonblocking_io_demo_%d", getpid());
