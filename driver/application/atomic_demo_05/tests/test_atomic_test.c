/**
 * @file test_atomic_test.c
 * @brief Unit tests for atomic demo application
 * @date 2026-05-10
 *
 * These tests focus on argument validation and error handling.
 * Full integration testing requires the actual atomic_demo driver.
 */
...
// Path to the atomic_test binary
#ifndef ATOMIC_TEST_BIN
#define ATOMIC_TEST_BIN "./atomic_test"
#endif
...
static int create_temp_dev(char* path, size_t path_size) {
    snprintf(path, path_size, "/tmp/test_atomic_%d", getpid());
...
// Helper: Run atomic_test and capture output
static int run_atomic_test(const char* dev_path, const char* value,
    ...
    if (pid == 0) {
        ...
        execl(ATOMIC_TEST_BIN, ATOMIC_TEST_BIN, dev_path, value, NULL);
...
static int test_help_on_wrong_args(void) {
    ...
    if (pid == 0) {
        ...
        execl(ATOMIC_TEST_BIN, ATOMIC_TEST_BIN, NULL);
        exit(1);
    } else {
        // Parent process
        int status;
...
static int test_help_content(void) {
    ...
    if (pid == 0) {
        ...
        ASSERT_STR_CONTAINS(output, "read");
        ASSERT_STR_CONTAINS(output, "set");
        ASSERT_STR_CONTAINS(output, "add");
        ASSERT_STR_CONTAINS(output, "sub");
        ASSERT_STR_CONTAINS(output, "inc");
        ASSERT_STR_CONTAINS(output, "dec");
    }
    return 1;
}
...
static int test_nonexistent_device(void) {
    char output[256];
    int result = run_atomic_test("/nonexistent/path/that/does/not/exist", "read",
...
// Test: Valid command 'read' is accepted (opens file successfully)
static int test_valid_read(void) {
    ...
    char output[256];
    int result = run_atomic_test(temp_path, "read", output, sizeof(output));
...
// Test: Valid command 'set' with value is accepted
static int test_valid_set_with_value(void) {
    char temp_path[256];
    int fd = create_temp_dev(temp_path, sizeof(temp_path));
    ASSERT_TRUE(fd >= 0);
    close(fd);
    ...
    char output[256];
    int result = run_atomic_test(temp_path, "set 42", output, sizeof(output));
...
// Test: 'set' command without value should fail
static int test_set_without_value(void) {
    char temp_path[256];
    int fd = create_temp_dev(temp_path, sizeof(temp_path));
    ASSERT_TRUE(fd >= 0);
    close(fd);
    ...
    char output[256];
    int result = run_atomic_test(temp_path, "set", output, sizeof(output));

    cleanup_temp_dev(temp_path);
    ASSERT_EQ(result, 1);  // Should fail
    ASSERT_STR_CONTAINS(output, "requires a value argument");
    return 1;
}
...
static int test_unknown_command(void) {
    ...
    char output[256];
    int result = run_atomic_test(temp_path, "unknown_command", output, sizeof(output));
...
int main(int argc, char* argv[]) {
    ...
    printf("========================================\n");
    printf("Atomic Demo Unit Tests\n");
    printf("Binary: %s\n", ATOMIC_TEST_BIN);
    printf("========================================\n");
    printf("\nNOTE: Full read/write cycle tests require actual\n");
    printf("atomic_demo driver hardware. These tests validate\n");
    ...
    if (tests_failed == 0) {
        ...
        printf("  1. Load the atomic_demo driver\n");
        printf("  2. Run: ./atomic_test read\n");
        printf("  3. Run: ./atomic_test set 100\n");
    }
