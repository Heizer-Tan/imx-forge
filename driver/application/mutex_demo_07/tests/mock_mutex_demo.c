/**
 * @file mock_mutex_demo.c
 * @brief Mock mutex demo device implementation for testing
 * @date 2026-05-10
 */

#include "mock_mutex_demo.h"
#include <errno.h>
#include <fcntl.h>
#include <stdio.h>
#include <stdlib.h>
...
mock_mutex_dev_t* mock_mutex_create(void) {
    mock_mutex_dev_t* dev = calloc(1, sizeof(mock_mutex_dev_t));
    if (!dev) {
        return NULL;
    }

    // Create temporary file
    snprintf(dev->temp_path, sizeof(dev->temp_path), "/tmp/mutex_demo_%d", getpid());
