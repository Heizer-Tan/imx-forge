/**
 * @file test_buffer_test.c
 * @brief Unit tests for blocking I/O demo application
 * @date 2026-05-10
 */
...
#ifndef BUFFER_TEST_BIN
#define BUFFER_TEST_BIN "./buffer_test"
#endif
...
static char* create_temp_dev(void) {
    ...
    snprintf(path, 256, "/tmp/test_blocking_%d", getpid());
...
static int run_test_binary(const char* dev_path, const char* arg,
    ...
    if (pid == 0) {
        ...
        close(pipefd[1]);
        execl(BUFFER_TEST_BIN, BUFFER_TEST_BIN, dev_path, arg, NULL);
        exit(1);
    }
...
static int test_help_on_wrong_args(void) {
    ...
    if (pid == 0) {
        ...
        dup2(pipefd[1], STDOUT_FILENO);
        close(pipefd[1]);
        execl(BUFFER_TEST_BIN, BUFFER_TEST_BIN, NULL);
        exit(1);
    }
...
static int test_nonexistent_device(void) {
    char output[256];
    int result = run_test_binary("/nonexistent/blocking", "read 10", output, sizeof(output));
    ASSERT_EQ(result, 1);
    return 1;
}
...
static int test_valid_command(void) {
    ...
    char output[256];
    int result = run_test_binary(path, "read 10", output, sizeof(output));
