...
- **构建脚本** —— 一键构建 U-Boot、Linux 内核、BusyBox Rootfs
- **教程文档** —— 从工具链到系统调试的完整学习路径
- **第三方源码** —— Git Submodule 管理的 U-Boot、Linux、BusyBox
...
│         ↓                                                     │
│  Linux Kernel (linux-imx) → zImage + .dtb                    │
│         ↓                                                     │
...
| Docker | ✅ 支持 | 可使用项目提供的 DevContainer |
...
```bash
# 克隆项目并初始化子模块
git clone --recurse-submodules https://github.com/yourusername/imx-forge.git
...
- **电源 LED** (红色): 常亮表示 5V 电源正常
- **系统 LED** (绿色): 系统正常运行时应该有特定闪烁模式（取决于板子设计）
...
1. **深入学习** —— 阅读项目教程文档
   ...
   - [内核教程](document/tutorial/kernel/01_kernel_overview.md)
...
4. **驱动开发** —— 学习编写 Linux 驱动程序
   - 参考 [驱动基础教程](document/tutorial/kernel/07_driver_basic.md)
...
1. 查看 [常见问题](#常见问题) 章节
2. 搜索项目的 [Issues](https://github.com/yourusername/imx-forge/issues)
